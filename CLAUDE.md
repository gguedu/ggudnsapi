...
- Run a single Vitest file when tests are added: `pnpm exec vitest run path/to/file.test.ts`
- Run tests matching a name when tests are added: `pnpm exec vitest run -t "test name"`

There is no lint script configured in `package.json`. The TypeScript config is strict and includes `src/**/*.ts` plus `test/**/*.ts`.
...
- `CREDENTIALS_ENCRYPTION_KEY` is an optional but recommended Wrangler secret used to encrypt Cloudflare API credentials before storing them in KV.
...
## External setup summarized from README
...
- The recommended secret setup command is `pnpm wrangler secret put CREDENTIALS_ENCRYPTION_KEY`.
